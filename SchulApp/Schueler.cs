    ...
    public partial class Schueler : CustomForm
    {
        ...
        private void KlassenLaden()
        {
            try
            {
                ...
                var klassen = context.Klassen.AsNoTracking().OrderBy(k => k.KlassenId)
                    .Select(k => new { k.KlassenId, k.Bezeichnung }).ToList();
        ...
        private async Task SchuelerListeAktualisierenAsync()
        {
            try
            {
                ...
                using SchulAppContext context = new SchulAppContext();
                var klassen = context.Klassen.AsNoTracking()
                    .ToDictionary(k => k.KlassenId, k => k.Bezeichnung);

                var anzeige = schueler.OrderBy(s => s.SchuelerId).Select(s => new
                {
                    s.SchuelerId,
                    s.Name,
                    s.KlasseId,
                    Klasse = klassen.TryGetValue(s.KlasseId, out string? bezeichnung)
                        ? bezeichnung : "Unbekannt"
                }).ToList();

                studentGrid.DataSource = anzeige;
                if (studentGrid.Columns.Contains("KlasseId")) studentGrid.Columns["KlasseId"].Visible = false;
                ...
                if (studentGrid.Columns.Contains("Name")) studentGrid.Columns["Name"].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
                if (studentGrid.Columns.Contains("Klasse")) studentGrid.Columns["Klasse"].Width = 130;
                studentGrid.ClearSelection();
                AuswahlZuruecksetzen();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Die Schüler konnten nicht über SOAP geladen werden.\n\n" + ex.Message);
            }
        }

        private static void AuditBenutzerSetzen(ServiceReference1.SchuelerModel schueler)
        {
            schueler.AuditUserName = BenutzerSession.Benutzername ?? string.Empty;
            schueler.AuditUserRole = BenutzerSession.Rolle ?? string.Empty;
        }

        private async void OKnewStudent_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                var schueler = new ServiceReference1.SchuelerModel
                {
                    ...
                    KlasseId = Convert.ToInt32(newStudentKlasse.SelectedValue)
                };
                AuditBenutzerSetzen(schueler);

                bool erstellt = await soapClient.AddSchuelerAsync(schueler);
                if (!erstellt)
                {
            ...
            {
                MessageBox.Show("Der Schüler konnte nicht über SOAP gespeichert werden.\n\n" + ex.Message);
            }
        }

        private async void showStudents_Click(object sender, EventArgs e)
        {
            KlassenLaden();
            await SchuelerListeAktualisierenAsync();
        }
        ...
        {
            if (studentGrid.SelectedRows.Count == 0) return;
            DataGridViewRow row = studentGrid.SelectedRows[0];
            if (row.Cells["SchuelerId"].Value == null) return;
        ...
        private async void OKchangeName_Click(object sender, EventArgs e)
        {
            if (nurHinzufuegen) return;
            ...
            try
            {
                var schueler = new ServiceReference1.SchuelerModel
                {
                    ...
                    KlasseId = Convert.ToInt32(editKlasse.SelectedValue)
                };
                AuditBenutzerSetzen(schueler);

                bool bearbeitet = await soapClient.UpdateSchuelerAsync(schueler);
                if (!bearbeitet)
                {
                    MessageBox.Show("Der Schüler wurde nicht gefunden oder konnte nicht bearbeitet werden.");
                    await SchuelerListeAktualisierenAsync();
                    return;
                }
            ...
            {
                MessageBox.Show("Der Schüler konnte nicht über SOAP bearbeitet werden.\n\n" + ex.Message);
            }
        }

        private async void OKdeleteBtn_Click(object sender, EventArgs e)
        {
            if (nurHinzufuegen) return;
            if (ausgewaehlteSchuelerId == null)
            {
            ...
            DialogResult antwort = MessageBox.Show(
                ...
                "Schüler löschen", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            if (antwort != DialogResult.Yes) return;

            try
            {
                bool geloescht = await soapClient.DeleteSchuelerMitAuditAsync(
                    ausgewaehlteSchuelerId.Value,
                    BenutzerSession.Benutzername ?? string.Empty,
                    BenutzerSession.Rolle ?? string.Empty);

                MessageBox.Show(geloescht ? "Schüler wurde gelöscht." : "Der Schüler wurde nicht gefunden.");
                await SchuelerListeAktualisierenAsync();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Der Schüler konnte nicht über SOAP gelöscht werden.\n\n" + ex.Message);
            }
        }
